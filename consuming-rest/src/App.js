...
class App extends Component {
  ...
  render() {
    ...
    const showLeagueTable = this.state.showLeagueTable;
    const selectedLeague = this.state.selectedLeague;
    const listItems = items.map((item) =>
        <li key={item.identifier} onClick={() => this.onSelect(item)}>
    ...
    return (
        <div className="App">
          ...
          <br/>
          {showLeagueTable && selectedLeague && <LeagueTable league={selectedLeague} />}
        </div>
    );
  }
  getItems() {
  ...
  onSelect(league) {
    ...
    this.itemService.getLeagueTable(league).then(league => {
          this.setState({
