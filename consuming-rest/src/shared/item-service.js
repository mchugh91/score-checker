...
class ItemService {
    ...
    async getLeagueTable(league) {
        // return fetch(this.config.LEAGUE_TABLE_URL)
        return fetch("http://soccer.sportsopendata.net/v1/leagues/" + league.league_slug +"/seasons/19-20")
            .then(response => {
                if (!response.ok) {
                    this.handleResponseError(response);
                }
            ...
            .then(json => {
                ...
                const items = json.data.standings;
                return items;
            })
            .catch(error => {
                this.handleError(error);
